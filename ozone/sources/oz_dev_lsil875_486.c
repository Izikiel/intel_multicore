...
#define ONEATATIME 1 // 0			/* 0: queue multiple requests (per scsi-id) at a time */
...

{
  ...
  while (crash_rp -> rp_next_va != crash_rp) {
    ...
    if (OZ_HW_DATEBIN_TST (crash_rp -> rp_timesout)) {					/* see if timeout parameter given */
      now = oz_hw_tod_getnow ();							/* ok, see what time it is now */
...

{
  ...
  if (rpblocksize > (1 << OZ_HW_L2PAGESIZE)) return (NULL);	/* this is the largest that OZ_KNL_PCMALLOC can do */
  rp = OZ_KNL_PCMALLOC (rpblocksize);				/* allocate the block */
  return (rp);
}
...

{
  ...
  if (doiopp -> timeout != 0) {
    rp -> rp_timesout  = oz_hw_tod_getnow ();
...

{
  ...
  if ((dstat & 0x04) && (regs -> dsps == 0x00000096)) {
	...
	oz_hw_tod_getnow (), devex -> name, scsi_id, regs -> scratcha0 ^ regs -> scratcha2, queueflg, regs -> scratcha0);
#endif
...

{
  ...
  if (!crash_inprog) {
    ...
    when  = oz_hw_tod_getnow ();					// see what time it is right now
