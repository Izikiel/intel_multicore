...
/************************************************************************/
/*									*/
/*  Communications port driver for 486's				*/
/*									*/
...
typedef struct { OZ_Devunit *devunit;			/* comport's devunit */
                 ...
                 OZ_Smplock *smplock_kb;		/* irq level's smp lock */
                 OZ_Hw486_irq_many irqmany;		/* irq service link block */
...
static uLong comport_486_assign (OZ_Devunit *devunit, void *devexv, OZ_Iochan *iochan, void *chnexv, OZ_Procmode procmode);
static int comport_486_deassign (OZ_Devunit *devunit, void *devexv, OZ_Iochan *iochan, void *chnexv);
static void comport_486_abort (OZ_Devunit *devunit, void *devexv, OZ_Iochan *iochan, void *chnexv, OZ_Ioop *ioop, void *iopexv, OZ_Procmode procmode);
static uLong comport_486_start (OZ_Devunit *devunit, void *devexv, OZ_Iochan *iochan, void *chnex, OZ_Procmode procmode, 
                                OZ_Ioop *ioop, void *iopexv, uLong funcode, uLong as, void *ap);

static OZ_Devfunc comport_486_functable = { sizeof (Devex), sizeof (Chnex), sizeof (Iopex), 0, NULL, NULL, NULL, comport_486_assign, 
                                            comport_486_deassign, comport_486_abort, comport_486_start, NULL };
...
static int uartinterrupt (void *devexv, OZ_Mchargs *mchargs);
...

{
  ...
  if (!initialized) {
    oz_knl_printk ("oz_dev_comport_486_init\n");

    /* Set up driver database entries */

    devclass  = oz_knl_devclass_create (OZ_IO_COMPORT_CLASSNAME, OZ_IO_COMPORT_BASE, OZ_IO_COMPORT_MASK, "oz_dev_comport_486");
    devdriver = oz_knl_devdriver_create (devclass, "oz_dev_comport_486");
...
static uLong comport_486_start (OZ_Devunit *devunit, void *devexv, OZ_Iochan *iochan, void *chnexv, OZ_Procmode procmode, 
                                OZ_Ioop *ioop, void *iopexv, uLong funcode, uLong as, void *ap)

{
  ...
  switch (funcode) {
    ...
    case OZ_IO_FS_DISMOUNT: {
      ...
      oz_hw486_outb (0, devex -> iobase + IER);			/* clear interrupt enables */
...

{
  ...
  if (comport_486_functable.chn_exsize < sizeof (Chnex) + devex -> comport_setup.class_functab -> chn_exsize) {
    comport_486_functable.chn_exsize = sizeof (Chnex) + devex -> comport_setup.class_functab -> chn_exsize;
  }
  if (comport_486_functable.iop_exsize < sizeof (Iopex) + devex -> comport_setup.class_functab -> iop_exsize) {
    comport_486_functable.iop_exsize = sizeof (Iopex) + devex -> comport_setup.class_functab -> iop_exsize;
  }
  ...
  oz_hw486_outb (devex -> lcr | LCR_DLAB, devex -> iobase + LCR);		/* set line params, select DLH/DLL registers */
  oz_hw486_outb (devex -> dl, devex -> iobase + DLL);				/* program divisor latch (line speed) */
  oz_hw486_outb (devex -> dl >> 8, devex -> iobase + DLH);
  oz_hw486_outb (devex -> lcr, devex -> iobase + LCR);				/* select RBR/THR registers */
  oz_hw486_outb (0, devex -> iobase + MCR);					/* make sure loopback is clear */
  oz_hw486_outb (FCR_RCV_RES | FCR_XMT_RES | devex -> rxfifolevel, devex -> iobase + FCR); /* set receive fifo threshold = 1 char */
  oz_hw486_outb (IER_DR | IER_TR | IER_LS | IER_MS, devex -> iobase + IER);	/* set interrupt enables */
...

{
  ...
  switch (iir & 0x07) {
    ...
    case 4: {
      ...
      nchars = 0;
      while ((lsr = oz_hw486_inb (iobase + LSR)) & (LSR_DR | LSR_OE | LSR_PE | LSR_FE | LSR_BI)) {
        if (lsr & LSR_DR) {
          rbr = oz_hw486_inb (iobase + RBR);
      ...
      if (newfifolevel != devex -> rxfifolevel) {
        ...
        oz_hw486_outb (newfifolevel, iobase + FCR);
      }
...

{
  ...
  while (devex -> sendbeep || ((devex -> writesize > 0) && !(devex -> suspwriteflag))) {
    lsr = oz_hw486_inb (iobase + LSR);					// check line status register
    if (!(lsr & LSR_TE)) break;						// exit loop if transmitter bussy
    if (devex -> sendbeep) {						// see if we need to send a beep
      oz_hw486_outb (BEEPCODE, iobase + THR);				// if so, send it
    ...
    oz_hw486_outsb (i, devex -> writebuff, iobase + THR);		// fill the fifo
...

{
  ...
  devunit = oz_knl_devunit_create (devdriver, unitname, unitname, &comport_486_functable, 0, oz_s_secattr_sysdev);
  ...
  devex -> irqmany.entry = uartinterrupt;
  devex -> irqmany.param = devex;
  devex -> irqmany.descr = devex -> name;
  devex -> smplock_kb    = oz_hw486_irq_many_add (irqlevel, &(devex -> irqmany));
  ...
  devex -> comport_setup.port_lkprm        = devex -> smplock_kb;
...

{
  ...
  olddata = oz_hw486_inb (iobase + MCR);
  oz_hw486_outb (0x10, iobase + MCR);
  if ((oz_hw486_inb (iobase + MSR) & 0xF0) != 0) return (NULL);
  oz_hw486_outb (0x1F, iobase + MCR);
  if ((oz_hw486_inb (iobase + MSR) & 0xF0) != 0xF0) return (NULL);
  oz_hw486_outb (olddata, iobase + MCR);

  /* Look for the scratch register */

  olddata = oz_hw486_inb (iobase + SCR);
  oz_hw486_outb (0x55, iobase + SCR);
  if (oz_hw486_inb (iobase + SCR) != 0x55) return ("8250");
  oz_hw486_outb (0xAA, iobase + SCR);
  if (oz_hw486_inb (iobase + SCR) != 0xAA) return ("8250");
  oz_hw486_outb (olddata, iobase + SCR);

  /* Check to see if there's a FIFO */

  oz_hw486_outb (1, iobase + FCR);			/* enable the fifo */
  olddata = oz_hw486_inb (iobase + IIR);		/* read the interrupt ident register */
  oz_hw486_outb (0, iobase + FCR);			/* disable the fifo */
...
/* Must be at exactly softint level for this to work, as */
/*   that is what is required by the oz_hw486_irq_hits   */
/*   stuff                                               */

static uLong check_irq_mask (void *irqmaskv);
...

{
  ...
  for (retries = 5; -- retries > 0;) {
    while (oz_hw_atomic_set_long (&oz_hw486_irq_hits_lock, 1) != 0) {}
    oz_hw486_outb (0, iobase + IER);			/* disable all uart interrupts */
    while (!(oz_hw486_inb (iobase + LSR) & 0x20)) {}	/* wait for the THR to be empty */
    oz_hw486_outb (0x0F, iobase + MCR);			/* connect uart to irq line */
    oz_hw486_irq_hits_mask = 0;
    oz_hw486_outb (2, iobase + IER);			/* enable uart to request interrupt */
    new_irq_mask = oz_hw_stl_microwait (500, check_irq_mask, (void *)irqmask); /* wait up to 500uS for an IRQ 3 or 4 interrupt */
    oz_hw486_irq_hits_lock = 0;				/* let someone else use oz_hw486_irq_hits_mask */
    oz_hw486_outb (0, iobase + IER);			/* turn off uart interrupts */
    if (new_irq_mask != 0) {				/* maybe we didn't get any bits */
      for (hi = 0; hi < 16; hi ++) {			/* loop through all 16 bits of the mask */
        if (new_irq_mask & 1) {				/* see if the bit is set */
          if (new_irq_mask != 1) break;			/* try again if more than one bit is set */
          return (hi);					/* just the one bit, return bit number = IRQ number*/
        }
        new_irq_mask /= 2;
      }
    }
...
{
  return (oz_hw486_irq_hits_mask & (uLong)irqmaskv);
}
