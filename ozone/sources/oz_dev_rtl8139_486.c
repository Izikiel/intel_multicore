...
struct Devex { OZ_Devunit *devunit;		/* devunit pointer */
               ...
               uLong rxring_paddr;		/* receive ring base physical address (RX_RING_TOTAL bytes) */
...
struct Iopex { Iopex *next;				/* next in list of requests */
               ...
               uLong txbufpa;				/* transmit buffer physical address */
             };
...
static int rtl_interrupt (void *rtlv, OZ_Mchargs *mchargs);
...
#define ether_inb(iobase,offset) oz_hw486_inb (iobase - 1 + offset)
#define ether_inw(iobase,offset) oz_hw486_inw (iobase - 1 + offset)
#define ether_inl(iobase,offset) oz_hw486_inl (iobase - 1 + offset)

#define ether_outb(value,iobase,offset) oz_hw486_outb (value, iobase - 1 + offset)
#define ether_outw(value,iobase,offset) oz_hw486_outw (value, iobase - 1 + offset)
#define ether_outl(value,iobase,offset) oz_hw486_outl (value, iobase - 1 + offset)

#else

#define ether_inb(iobase,offset) (*(uByte *)(iobase + offset))
#define ether_inw(iobase,offset) (*(uWord *)(iobase + offset))
#define ether_inl(iobase,offset) (*(uLong *)(iobase + offset))

#define ether_outb(value,iobase,offset) *(uByte *)(iobase + offset) = value
#define ether_outw(value,iobase,offset) *(uWord *)(iobase + offset) = value
#define ether_outl(value,iobase,offset) *(uLong *)(iobase + offset) = value
...

{
  ...
  devclass  = oz_knl_devclass_create (OZ_IO_ETHER_CLASSNAME, OZ_IO_ETHER_BASE, OZ_IO_ETHER_MASK, "rtl8139_486");
  devdriver = oz_knl_devdriver_create (devclass, "rtl8139_486");
...

{
  ...
  *(uWord *)(enaddr + 4) = ether_inl (iobase, IO_L_IDR4_5);
  ...
  devex -> irq_many.entry = rtl_interrupt;
...

{
  ...
  switch (funcode) {
    ...
    case OZ_IO_ETHER_TRANSMIT: {
      ...
      sts = oz_knl_misc_sva2pa (txbuf, &txbufppn, &txbufppo);
      if (sts < sizeof *txbuf) oz_crash ("oz_dev_rtl8139 transmit: txbuf %p not physically contiguous", txbuf);
      iopex -> txbufpa = (txbufppn << OZ_HW_L2PAGESIZE) + txbufppo;
...

{
  ...
  /* Get physical and virtual starting addresses */

  devex -> rxring_paddr = phypages << OZ_HW_L2PAGESIZE;
  devex -> rxring_vaddr = OZ_HW_VPAGETOVADDR (sysvpage);

  oz_knl_printk ("oz_dev_rtl8139: %s rcv ring phys addr %p, virt addr %p\n", 
	devex -> name, devex -> rxring_paddr, devex -> rxring_vaddr);
...

{
  ...
  while (devex -> txfree != 0) {							/* repeat while transmitter can do more */
    ...
    ether_outl (txreq -> txbufpa, devex -> iobase, IO_L_TSAD0 + (i * 4));		/* start transmitting */
...
#define in(port) oz_hw486_inb (port - 1)
#define in16(port) oz_hw486_inw (port - 1)
#define in32(port) oz_hw486_inl (port - 1)
#define out(port,data) oz_hw486_outb (data, port - 1)
#define out16(port,data) oz_hw486_outw (data, port - 1)
#define out32(port,data) oz_hw486_outl (data, port - 1)
...

{
  ...
  for (i = 0; i < NUM_TX_DESC; i ++) {			// scan through transmit slots
    if (rtl -> txreqip[i] != NULL) {			// see if request was in progress
      oz_knl_iodonehi (rtl -> txreqip[i] -> ioop, OZ_ABORTED, NULL, NULL, NULL); // abort if so
  ...
  out32 (rtl -> iobase + RxBuf, rtl -> rxring_paddr);
